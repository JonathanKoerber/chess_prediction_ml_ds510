...
print("Stockfish binary path: ", STOCKFISH_BINARY_PATH)

parser = argparse.ArgumentParser()



parser.add_argument("--games", default=3000, type=int)
...
class Game:
    ...
    def __init__(self, threads=2, ELO=2500, thinking_time=50):
    ...
    def run(self):
        ...
        while True:
            ...
            if num_moves == 1:
                # for the first move
                index = random.randint(0, len(self.first_moves)-1)
...
if __name__=="__main__":
    ...
    #######
    # this will create a csv file with the FEN positions and the moves
    #######

    # f3 = open("dataset_fen.csv", 'a')
    # f3.write("Board_FEN, Best_Move\n ")
    # for _ in tqdm(range(args.games)):
    #     start = time.time()
    #     moves, fen_positions = game.run()
    #     stop = time.time()
    #     times.append(stop - start)
    #     dataset_3 = map(lambda x: ",".join(x), zip(fen_positions, moves))  # Changed delimiter to ','
    #     f3.write("\n".join(dataset_3) + "\n")
    # print(np.average(times))

    #######
    # this will create a txt file with the FEN positions and the moves
    #######
